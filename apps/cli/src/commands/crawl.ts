...
import type { CrawlOptions, CrawlResult, CrawlStatusResult } from '../types/crawl';
...
export async function executeCrawl(
  ...
  try {
    ...
    if (wait) {
      // Set polling options (SDK waitForCrawlCompletion expects seconds,
      // not milliseconds — it multiplies by 1000 internally).
      ...
      if (options.progress) {
        ...
        const pollMs = (crawlOptions.pollInterval || 5) * 1000;
 ...
 */
function formatCrawlStatus(data: any): string {
  if (!data) return '';
